...
function AdminPanel({ user }) {
  return <section className="page-body"><div className="module-heading"><div><span className="eyebrow">Organisation Management</span><h2>Admin Panel</h2><p>Open the complete Django administration system for users, roles, departments, employees, payroll, and settings.</p></div><span className="admin-badge">{user.user_type || 'ADMIN'}</span></div><div className="admin-grid"><article className="panel"><span className="eyebrow">Django Admin</span><h3>Complete administration</h3><p className="admin-value">{user.first_name} {user.last_name}</p><p className="muted">Use the full backend administration interface.</p><button className="primary-button" onClick={() => { window.location.href = getAdminUrl() }}>Open Django Admin</button></article><article className="panel"><span className="eyebrow">Backend</span><h3>Connected services</h3><p className="admin-value">Django + PostgreSQL</p><p className="muted">React uses the live REST API for organisation data.</p></article></div></section>
}
