...
import { clearSession, deleteRecord, fetchDashboard, fetchModule, getStoredUser, login, setSession } from './api'
...
function Placeholder({ page }) {
  ...
  function remove(id) { deleteRecord(page, id).then(() => fetchModule(page).then(setResult)) }
  return <section className="page-body"><div className="module-heading"><div><span className="eyebrow">Organisation Management</span><h2>{page}</h2><p>Manage live records in the React workspace.</p></div><button className="primary-button" onClick={() => alert(`Use the ${page} form to add a record.`)}>+ Add record</button></div>{error && <div className="api-warning">{error}</div>}<div className="data-table">{rows.length ? rows.map((row) => <div className="data-row" key={row.id}><strong>{row.full_name || row.title || row.name || row.employee_id || row.id}</strong><span>{row.email || row.department_name || row.display_level || row.status || 'Record'}</span><span>{row.role_title || row.code || row.date || row.amount || ''}</span><button className="delete-button" onClick={() => remove(row.id)} aria-label={`Delete ${row.full_name || row.title || row.name || 'record'}`}>Delete</button></div>) : <p className="muted empty-list">No records returned yet.</p>}</div></section>
}

function AdminPanel({ user }) {
  return <section className="page-body"><div className="module-heading"><div><span className="eyebrow">Organisation Management</span><h2>Admin Panel</h2><p>Manage users, organisation data, modules, and workspace access from one React administration page.</p></div><span className="admin-badge">{user.user_type || 'ADMIN'}</span></div><div className="admin-grid"><article className="panel"><span className="eyebrow">Access</span><h3>Current administrator</h3><p className="admin-value">{user.first_name} {user.last_name}</p><p className="muted">{user.email}</p></article><article className="panel"><span className="eyebrow">Storage</span><h3>Workspace data</h3><p className="admin-value">Local project data</p><p className="muted">Records are stored in this browser for this single React deployment.</p></article><article className="panel"><span className="eyebrow">Security</span><h3>Session status</h3><p className="admin-value">Authenticated</p><p className="muted">Use the profile menu to sign out.</p></article></div></section>
}
